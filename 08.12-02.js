const input = `cgdf eagcbf fc adefg eacdb fbedga geafcd efc dacfe fdgaecb | dcefbag dgcf fc daefc
 ...
 *
 * @typedef {Object} Line
 * @property {NumInput[]} numInputs
 * @property {string[]} valueInputs
 * @property {[NumInput | null, NumInput | null, NumInput | null, NumInput | null, NumInput | null, NumInput | null, NumInput | null, NumInput | null, NumInput | null, NumInput | null]} numberCodes
 */
...
function codeToBits(code) {
    return code.split('')
        .reduce(
            (acc, letter) => acc | (1 << letter.charCodeAt(0) - 'a'.charCodeAt(0)),
            0
        );
}
...
function setIdentification(line, input, number) {
    if (number >= 0) {
        input.number = number;
        line.numberCodes[number] = input;
    }
}
 ...
 */
const ANALYZERS = [
    function identifyByLength(line) {
        for (const input of line.numInputs) {
            const identified = [-1, -1, 1, 7, 4, -1, -1, 8][input.length];
            setIdentification(line, input, identified);
        }
    },
    function identifyByOneParity(line) {
        const oneBits = line.numberCodes[1]?.bits;
        if (!oneBits) {
            return;
        }
        ...
        for (const input of line.numInputs) {
            let identified = -1;
            const xorCount = countBits(input.bits ^ oneBits);

            if (input.length === 5 && xorCount === 3) {
                identified = 3;
            } else if (input.length === 6 && xorCount === 6) {
                identified = 6;
            }
    ...
    function identifyByFourParity(line) {
        const fourBits = line.numberCodes[4]?.bits;
        if (!fourBits) {
            return;
        }
        ...
        for (const input of line.numInputs) {
            let identified = -1;
            const orCount = countBits(input.bits | fourBits);

            if (input.length === 6 && orCount === 6) {
                identified = 9;
            }
    ...
    function identifyZero(line) {
        for (const input of line.numInputs) {
            if (input.number >= 0) {
                continue;
            }

            if ((input.bits | line.numberCodes[7].bits) === input.bits) {
                setIdentification(line, input, 0);
            }
        }
    },
    function identifyTwoAndFive(line) {
        for (const input of line.numInputs) {
            if (input.number >= 0) {
                continue;
            }

            const identified = (countBits(input.bits & line.numberCodes[4].bits) === 2)
                ? 2
                : 5;
...
const lines = input
    ...
    .map(line => line
        .split('|')
        .map(inputs => inputs.trim().split(' '))
    )
    .map(([numInputs, valueInputs]) => ({
        numInputs: numInputs.map(raw => ({ raw: orderLetters(raw), bits: codeToBits(raw), length: raw.length, number: -1 })),
        valueInputs,
        numberCodes: new Array(10).fill(null)
    }));

const analyzed = lines
    .map(analyze);

const numbers = analyzed
    .map(line => line.valueInputs
        .map(input => {
            const ordered = orderLetters(input);
            return line.numInputs.find(num => num.raw === ordered).number
        })
        .reduce(
            (acc, dec) => acc * 10 + dec,
            0
