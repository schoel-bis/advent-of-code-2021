(def input "cgdf eagcbf fc adefg eacdb fbedga geafcd efc dacfe fdgaecb | dcefbag dgcf fc daefc
...
(def lines (filter (comp not #(= 0 (count %))) (clojure.string/split-lines input)))
(def line-parts (map
    (partial map (partial re-seq #"\S+"))
    (map
        #(clojure.string/split % #" \| ")
        lines)))

(defn identify-by-length
    [segments]
    (case (count segments)
        2 1
        3 7
...
(apply
    ...
    (map
        (fn [line]
            (count (filter (comp not nil?) (map identify-by-length (nth line 1)))))
        line-parts))
